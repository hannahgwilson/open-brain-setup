...
/**
 * Canonical-table bridge config. When an extracted entity's canonical_name
 * exactly matches (case-insensitive) a row in the canonical table, populate
 * the entity's FK column. Lets the knowledge graph join to the structured
 * tables for queries like "what have we discussed about Marthe?".
 *
 * Skipped silently for:
 *   - entity_types not in this map
 *   - ambiguous matches (>1 canonical row with the same name)
 *   - FK already set to the matching id (no-op)
 *
 * Requires the bridge FK columns to exist on `entities`:
 *   - entities.contact_id      (added by family-calendar follow-on)
 *   - entities.location_id     (added by family-calendar follow-on)
 *   - entities.organization_id (added by schemas/organizations/schema.sql)
 *
 * If a column is missing the bridge call silently no-ops on PG error — the
 * worker keeps processing. Apply the schema to enable bridging.
 */
const ENTITY_BRIDGE: Record<string, { table: string; fkColumn: string }> = {
  person: { table: "contacts", fkColumn: "contact_id" },
  place: { table: "locations", fkColumn: "location_id" },
  organization: { table: "organizations", fkColumn: "organization_id" },
};

async function bridgeEntityToCanonical(
  entityId: number,
  canonicalName: string,
  entityType: string,
): Promise<void> {
  const bridge = ENTITY_BRIDGE[entityType];
  if (!bridge) return;

  const normalized = normalizeName(canonicalName);

  // Find candidate canonical rows by case-insensitive name match.
  // We deliberately don't filter by user_id here: entities are global
  // (shared dedup table) so the bridge can't disambiguate users at this layer.
  // Multi-user deployments should treat ambiguous matches as "skip".
  ...
  if (lookupError) {
    // Likely cause: canonical table doesn't exist yet, or FK column missing.
    console.warn(
      `bridgeEntityToCanonical: ${bridge.table} lookup failed for "${canonicalName}" — ${lookupError.message}`,
    );
    return;
  }

  if (!candidates || candidates.length === 0) return; // no match
  if (candidates.length > 1) return; // ambiguous, skip

  const canonicalId = candidates[0].id;

  // Check current FK value to avoid pointless writes.
  const { data: current } = await supabase
    .from("entities")
    .select(bridge.fkColumn)
    .eq("id", entityId)
    .single();

  if (current && (current as Record<string, unknown>)[bridge.fkColumn] === canonicalId) {
    return; // already linked
  }

  const { error: updateError } = await supabase
    .from("entities")
    .update({ [bridge.fkColumn]: canonicalId })
    .eq("id", entityId);

  if (updateError) {
    console.warn(
      `bridgeEntityToCanonical: failed to set entities.${bridge.fkColumn} for "${canonicalName}" — ${updateError.message}`,
    );
    return;
  }

  // Telemetry log — useful for debugging matchless extractions.
  void normalized; // (reserved if we later switch to normalized_name match)
}
...
Deno.serve(async (req) => {
  ...
  for (const item of claimed) {
    ...
    for (const entity of result.entities) {
      ...
      // Best-effort bridge to canonical tables (contacts/locations/organizations).
      // Non-fatal: any failure logs a warning and continues.
      await bridgeEntityToCanonical(entityId, entity.name, entity.type);
    }
